...
```sh
# Node 22+ (packages/* are .ts consumed via Node's type stripping)
...
npm --prefix .. run fetch-data          # or run scripts/fetchData.ts from repo root
```
...
- `src/i18n/` — i18next over the repo-root `public/locales/*.json` (verbatim)
